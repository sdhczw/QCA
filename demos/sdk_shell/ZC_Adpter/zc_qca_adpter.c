...
{
    u32 u32MagicFlag;
    tSuccessMode_Type  SnifferTmode;
    u8  u8SnifferResponse;
}QC_SnifferInfo;

QC_SnifferInfo g_struSnifferInfo = {0};
...
void QC_ReadDataFormFlash(void) 
{
    ...
    u32Offset = u32Offset - sizeof(QC_SnifferInfo);
    _ota_nvram_read_data(g_s32StorePartition, u32Offset, (u8 *)(&g_struSnifferInfo.u32MagicFlag), 4);
    if (ZC_MAGIC_FLAG == g_struSnifferInfo.u32MagicFlag)
    {   
        _ota_nvram_read_data(g_s32StorePartition, u32Offset, (u8 *)(&g_struSnifferInfo), sizeof(QC_SnifferInfo));
    }
    else
    {
        ZC_Printf("no sniffer");
    }
}
...
* History:
*************************************************/
void QC_SnifferSuccess(char *ssid, char *password, u8 response, tSuccessMode_Type tmode)
{
    u32 u32Offset = 0x40000 - sizeof(ZC_ConfigDB)-sizeof(QC_SnifferInfo);
    if (tmode == AIRKISS)
    {
        ZC_Printf("airkiss ");
    }
    ...
    g_struSnifferInfo.u32MagicFlag = ZC_MAGIC_FLAG;
    g_struSnifferInfo.u8SnifferResponse = response;
    g_struSnifferInfo.SnifferTmode = tmode;

    _ota_nvram_write_data(g_s32StorePartition, u32Offset, (u8 *)&g_struSnifferInfo, sizeof(QC_SnifferInfo));
    qcom_sys_reset();
}
...
void QC_SetNetwork()
{
    ...
    while(wifi_state != 4)
    {
        ...
        if (wifi_state == 4) 
        {
            ...
            if (ZC_MAGIC_FLAG == g_struSnifferInfo.u32MagicFlag)
            {
                if (g_struSnifferInfo.SnifferTmode == AIRKISS)
                {
                    udp_sniffer_response(10000,10,(char *)&g_struSnifferInfo.u8SnifferResponse,1);
                }
                else
                {
                    udp_sniffer_response(6001,10,(char *)&g_struSnifferInfo.u8SnifferResponse,1);
                }
            }
            QC_WakeUp();
            break;
